...
public class PagerSlidingTabStripExtends extends HorizontalScrollView {
	...
	private int selectedTabTextSize = 12;//新增：tab标题选中文字大小
	private int selectedTabTextColor = 0xFF888844;//新增：tab标题选中文字颜色
	private int selectedPosition;//ViewPager当前选中页面的Position
...
/*
		...
		int x1 = (int) ((lineRight - lineLeft) / 2 + lineLeft);
		int y1 = height - indicatorHeight;
		int x2 = x1 - triangleWidth/2;
		int y2 = height;
		int x3  = x1 + triangleWidth/2;
		int y3 = height;
		Path path = new Path();
		path.moveTo(x1, y1);
		path.lineTo(x2, y2);
